...
for node in config.proxmox_nodes:
    ...
    render_packer_file(root_path=root_folder, tmp_path=tmp_path, config_obj=config, node=node)
    render_user_data(root_path=root_folder, tmp_path=tmp_path, config_obj=config)
